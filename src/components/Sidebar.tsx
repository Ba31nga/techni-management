...
export default function Sidebar() {
  ...
  useEffect(() => {
    if (!loading && userId) fetchPages();
  }, [loading, userId, rolesKey]);
  ...
  const fetchPages = async () => {
    ...
    pagesSnap.forEach((doc) => {
      ...
      let hasRoleView = false;

      userRoles.forEach((role) => {
        const perm = data.permissions?.role?.[role];
        const order = roleOrderMap[role] ?? 999;
        if (perm?.view && order < bestOrder) {
          bestOrder = order;
          hasRoleView = true;
        }
      });

      if (hasRoleView) result.push(data);
    });
      ...
      <div className="flex items-center justify-between py-4">
        <div className="flex items-center gap-2">
          <img
            src="/logo.png"
            alt="School Logo"
            className={clsx("transition-all", "w-8 h-8", !open && "mx-auto")}
          />
          {open && (
